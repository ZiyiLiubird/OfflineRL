...
class AlgoTrainer(BaseAlgo):
    ...
    def train_transition(self, buffer):
        ...
        while True:
            ...
            logger.info('enter model for loop....')
    ...
    def _cql_update(self, batch_data, model_data, transition):
        ...
        random_actions = torch.rand(self.args['num_samples'], indices_large.sum(), action.shape[-1]).to(action) * 2 - 1

        action_dist = self.actor(m_obs[indices_large])
        sampled_actions = torch.stack([action_dist.rsample() for _ in range(self.args['num_samples'])], dim=0)

        random_next_actions = torch.rand(self.args['num_samples'], indices_large.sum(), action.shape[-1]).to(action) * 2 - 1
        
        #*******************
        #all_next_obs = torch.cat([next_obs, m_next_obs[indices_large]])
        next_action_dist = self.actor(m_next_obs[indices_large])
        ...
        repeated_obs = torch.repeat_interleave(m_obs[indices_large].unsqueeze(0), sampled_actions.shape[0], 0)
        ...
        repeated_next_obs = torch.repeat_interleave(m_next_obs[indices_large].unsqueeze(0), sampled_next_actions.shape[0], 0)
        ...
        if self.args['with_important_sampling']:
            ...
            _random_log_prob = torch.ones(self.args['num_samples'], indices_large.sum(), 1).to(sampled_q1) * action.shape[-1] * np.log(0.5)
